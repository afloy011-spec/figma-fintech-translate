...
figma.ui.onmessage = async (msg: any) => {
  ...
  if (msg.type === "create-frame") {
    ...
    const gap = 80;
    const origAbsX = (orig as FrameNode).absoluteTransform
      ? (orig as FrameNode).absoluteTransform[0][2] : orig.x;
    const origAbsY = (orig as FrameNode).absoluteTransform
      ? (orig as FrameNode).absoluteTransform[1][2] : orig.y;
